...
## Alcohol Content
  ...
  - Colorado
```{r, echo=FALSE}
boxplot(beers$ABV, main=toupper("Alcohol Content in Beers"), font.main=3, cex.main=1.2, xlab="Beers", ylab="Alcohol Content", font.lab=3, col="darkgreen") + theme_minimal()
```
...
- Oregon
```{r,echo=FALSE }
boxplot(beers$IBU, main=toupper("Bitterness in Beers"), font.main=3, cex.main=1.2, xlab="Beers", ylab="Bitterness", font.lab=3, col="darkgreen") + theme_minimal()
```
...
```{r, echo=FALSE }
grid(plot(combined$ABV, combined$IBU, main="Scatterplot Between Alcohol Content & Bitterness", xlab = "Alcohol Content", ylab = "Alcohol Bitterness")) + theme_minimal()
```
...
abline(brew_pop.lm, col = "red") + theme_minimal()
```
...
- ***We will recommend to target the states with less breweries regardless of the population, because we did not find any strong  correlation between the population and the amount of breweries.***
